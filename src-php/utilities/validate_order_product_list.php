 ...
 * @return array
 */
function validate_order_product_list(array $product_list): array
{
  $productIDs = [];
  foreach ($product_list as $product) {
    if (is_array($product)) {
      ...
      if (in_array($product['codproduct'], $productIDs)) {
        ...
        $productIDs[] = $product['codproduct'];
      }

      if (!array_key_exists('codproduct', $product)) {
        return array('message' => 'A product code is missing');
      }
      ...
      if (!array_key_exists('amountproduct', $product)) {
        return array('message' => 'A product amount is missing');
      }

      if (!filter_var(
        $product['amountproduct'],
        FILTER_VALIDATE_INT, ['options' => ['min_range' => '1', 'max_range' => '32767']])) {
      ...
      try {
        ...
        $query = $connection->prepare("SELECT codproduct FROM " . PRODUCTS .
          " WHERE codproduct = :codproduct AND productdeleted = 0 AND coduser = :coduser");
        ...
        if (!$result) {
          return array('message' => 'The product ' . $product['codproduct'] . ' does not exists or is deleted');
        }
