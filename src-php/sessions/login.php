 ...
 */
function security($renew = true): array|int
{
  $answer = -3;
  // User login check
 ...
 * @return array
 */
#[ArrayShape(['no_logged' => "string", 'forbidden' => "string", 'time' => "string"])]
function reason_no_session(int $security): array
{
  if ($security < -2) {
 ...
 * @return array
 */
function login(array $input_data): array
{
  // Input data control
  ...
  try {
    // SQL queries to search for matching users
    $connection = create_pdo_object();
